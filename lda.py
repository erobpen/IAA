...
def analyze_lda():
    ...
    try:
        ...
        plt.semilogy(merged['Year'], merged['Total_LDA_SP500'], label='Total Return S&P 500 (Div Reinvested)', color='#10b981', linewidth=2)
        plt.semilogy(merged['Year'], merged['Total_LDA_3x'], label='Total Return 3x Strategy (Div Reinvested)', color='#8b5cf6', linewidth=2)
        
        plt.title('Leverage Dividend Adjusted Total Return ($10k Initial)')
        ...
        for _, row in table_records.iterrows():
            table_data.append({
                'year': int(row['Year']),
                'total_sp500': f"${row['Total_LDA_SP500']:,.0f}",
                'total_3x': f"${row['Total_LDA_3x']:,.0f}",
